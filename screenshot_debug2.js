...
(async () => {
  ...
  try {
    ...
    // Click on the uploaded file in the sidebar to render the graph (if necessary)
    const fileItems = await page.locator('li.cursor-pointer').all();
    if (fileItems.length > 0) {
      console.log('Clicking on the uploaded file to render graph...');
      await fileItems[0].click();
      await page.waitForTimeout(3000);
    }

    await page.screenshot({ path: 'debug_graph2.png' });
    console.log('Screenshot saved to debug_graph2.png');
  } catch (e) {
